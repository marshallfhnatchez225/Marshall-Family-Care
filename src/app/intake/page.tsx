...
export default async function IntakePage(){
 ...
 return <AppShell active="intake"><div className="content workflow intake-page">
  ...
  <section><div className="section-heading"><div><p className="eyebrow">Step 2</p><h2>Send first-call documents</h2></div><p>Each packet uses a private link that expires after seven days.</p></div>
   ...
   <div className="intake-list">{cases.map(c=>{const caseDocs=documents.filter(d=>d.case_id===c.id);const prepared=Boolean(c.metadata.intake_prepared_at);return <article className="workflow-card intake-case" key={c.id}><div className="intake-case-head"><div><span className="eyebrow">{c.case_number}</span><h2>{caseName(c)}</h2><p>{String(c.metadata.family_email||'Email not recorded')} · {String(c.metadata.family_mobile||'Mobile not recorded')}</p></div><Link className="secondary" href={`/cases/${c.id}`}>Review case</Link></div><ActionForm action={pipelineAction} submit={prepared?'Prepare a new Google Voice link':'Prepare Google Voice message'}><input type="hidden" name="op" value="send-intake"/><input type="hidden" name="case_id" value={c.id}/><div className="document-picker">{firstCallDocuments.map(document=>{const record=caseDocs.find(d=>d.metadata.section===document.section);return <label className="document-choice" key={document.section}><input type="checkbox" name={`document.${document.section}`} defaultChecked/><span><strong>{document.label}</strong><small>{document.description}</small></span><em>{record?.status||'ready'}</em></label>})}</div><p className="form-note">This creates the private link and a ready-to-paste Google Voice message. It does not mark the case sent.</p></ActionForm>{prepared&&<ActionForm action={pipelineAction} submit="I sent it in Google Voice · move to Cases"><input type="hidden" name="op" value="graduate-intake"/><input type="hidden" name="case_id" value={c.id}/><p className="form-note">Use this only after Google Voice confirms the message was sent.</p></ActionForm>}</article>})}</div>
  </section>
 </div></AppShell>;
}
