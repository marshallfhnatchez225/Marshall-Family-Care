...
export default async function SettingsPage({searchParams}:{searchParams:Promise<{module?:string}>}){
  ...
  return <AppShell active="settings"><div className="content modulemodern settingsmodern">
    ...
    <section className="settingscallout"><div><h2>Email & Google Voice</h2><p>Configure automatic email, review Google Voice packet delivery, and review delivery status.</p></div><Link href="/settings/notifications">Manage delivery <ArrowRight size={14}/></Link></section>
    <section className="settingscallout"><div><h2>Google Voice sending bridge</h2><p>Track queued packet texts, verified sends, and attempts that need review.</p></div><Link href="/settings/voice-bridge">Open text delivery queue <ArrowRight size={14}/></Link></section>
