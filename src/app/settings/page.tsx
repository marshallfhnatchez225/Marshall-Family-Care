...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ module?: string }> }) {
  ...
  return <AppShell active="settings"><div className="content modulepage">
    ...
    <section className="settingsgrid">
      ...
      <article className="panel settingcard"><span className="settingicon"><Database size={20}/></span><div><h2>Supabase</h2><p>PostgreSQL, authentication, files, and organization-scoped access.</p><span className="badge"><CircleDashed size={12}/> Connection required</span></div></article>
    ...
    <section className="panel nextstep"><div><p className="eyebrow">Next connection</p><h2>Connect the shared Marshall database</h2><p>Marshall OS is live. Supabase is the next dependency required to replace representative dashboard records with operational data.</p></div><a className="primary" href="https://vercel.com/marshall-funeral-home/marshall-os/settings/environment-variables" target="_blank" rel="noreferrer">Open environment settings</a></section>
  </div></AppShell>;
}
